...
def main():
    ...
    # Model A: Model 1 has 38 parameters (incl intercept), Model 2 has 40, Model 3 has 50. Total A: 128
    # Model B: Model 1 has 38 parameters, Model 2 has 40, Model 3 has 50. Total B: 128
    # Total bootstrap rows should be 256.
    print(f"  Bootstrap coefficients rows: {bootstrap_rows} (Expected=256)")
    if bootstrap_rows != 256:
        struct_passed = False
        
    # 4. Feature counts and matrix dimensions
    print(f"  Feature correlations rows: {len(df_corr_rep)} (Expected=55 * 55 = 3025)")
    if len(df_corr_rep) != 3025:
        struct_passed = False
