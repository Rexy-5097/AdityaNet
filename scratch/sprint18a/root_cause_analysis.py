...
def run_parallel_bootstrap(X, y, feature_names, model_name, model_group, n_iterations=10000, n_jobs=8):
    ...
    Uses n_jobs=8 to target performance cores and prevent efficiency core bottleneck.
    """
...
def main():
    ...
    for group, models in models_to_fit.items():
        for name, X, names, y in models:
            df_boot, boot_meta = run_parallel_bootstrap(X, y, names, name, f"Model_{group}", n_iterations=10000, n_jobs=8)
