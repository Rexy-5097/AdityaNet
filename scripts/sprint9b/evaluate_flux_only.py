"""
scripts/sprint9b/evaluate_flux_only.py

Sprint 9B-B: Production-Parity Evaluation Correction
Evaluation: Flux-Only Model
...
MODEL_PATH       = os.path.join(OUTPUT_DIR, "suryanet_flux_only.pt")
...
METRICS_PATH     = os.path.join(OUTPUT_DIR, "metrics_flux_only_corrected.json")
...
def main():
    logger.info("Initializing Sprint 9B-B: Corrected Flux-Only Evaluation")
    ...
    logger.info(f"Flux-only TSS = {tss:.4f} | F1 = {f1:.4f} | ROC-AUC = {roc_auc:.4f}")
