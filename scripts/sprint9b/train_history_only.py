...
Experiment B: History-Only Retraining
...
# Save paths
BEST_CKPT_PATH   = os.path.join(OUTPUT_DIR, "best_history_only.pt")
LAST_CKPT_PATH   = os.path.join(OUTPUT_DIR, "suryanet_history_only.pt")
CALIBRATOR_PATH  = os.path.join(OUTPUT_DIR, "calibrator_history_only.pkl")
LOG_PATH         = os.path.join(OUTPUT_DIR, "training_log_history_only.json")
...
def main():
    logger.info("Initializing Sprint 9B Experiment B: History-Only Retraining")
    ...
    logger.info(f"History-only feature set ({len(feature_cols)} features): {feature_cols}")

    # Load datasets (strict split isolation, never load test split here)
    logger.info("Loading training dataset...")
    train_ds = SolarFlareWindowDataset(TRAIN_PARQUET, seq_len=SEQ_LEN, feature_cols=feature_cols, split_name="train_history")
    pos_rate = float(train_ds.get_labels().mean())
    
    logger.info("Loading validation dataset...")
    val_ds = SolarFlareWindowDataset(VAL_PARQUET, seq_len=SEQ_LEN, feature_cols=feature_cols, split_name="val_history")
    ...
    logger.info("History-only retraining training pipeline finished successfully.")
