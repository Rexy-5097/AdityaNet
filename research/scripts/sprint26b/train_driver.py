"""
scripts/sprint26/train_driver.py

Sprint 26 — faithful, parameterized reproduction of the frozen Version 1 trainer
(app/services/ml/trainer.py). Reuses the frozen FocalLoss, PatchTST, and dataset
code unchanged; only the training LOOP is reimplemented so that the three
variables the stock trainer hardcodes (focal alpha, focal gamma, cosine T_max)
become protocol parameters. Every other value defaults to the frozen baseline.

Used identically for all seven Sprint 25 configurations (B0 + E1..E6); each
invocation changes exactly ONE variable from the baseline, per the protocol.

Writes under artifacts/sprint26/runs/<run_id>/:
  best.pt, history.json, run_meta.json
"""
...
FEATURE_COLS = json.load(open(os.path.join("artifacts", "feature_columns.json")))
...
def main():
    ...
    # protocol baseline defaults (02_retraining_protocol.md)
    ...
    t0 = time.time()
    set_seed(args.seed)
    out = os.path.join("artifacts", "sprint26b", "runs", args.run_id)
    ...
    # loader (dataset.py:166-169 — shuffle distributes positives; shuffle=False
    # is only for ordered test eval). num_workers=2 as in scripts/train_patchtst.py.
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for i, (X, y) in enumerate(train_loader):
            if i >= args.steps_per_epoch: break
        ...
        if m["tss"] > best_tss:
            ...
            torch.save({"epoch": epoch, "val_tss": best_tss, "best_threshold": float(thr),
                        "model": model.state_dict(), "seed": args.seed}, os.path.join(out, "best.pt"))
        else:
            patience_ctr += 1
