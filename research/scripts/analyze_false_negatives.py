"""
scripts/analyze_false_negatives.py

Sprint 6 — Task C: False Negative Analysis
...
Computes feature summary statistics and runs Mann-Whitney U and KS tests (FN vs TP, FN vs TN).
Profiles predicted probability (cal_prob) distribution for FN windows.
Saves to artifacts/fn_statistics.json.
"""
...
OUTPUT_PATH = os.path.join("artifacts", "fn_statistics.json")
...
def main():
    ...
    fn_df = aligned_features[(~y_pred) & (y_true == 1)]
    tn_df = aligned_features[(~y_pred) & (y_true == 0)]

    logger.info(f"Group sizes: TP={len(tp_df)}, FN={len(fn_df)}, TN={len(tn_df)}")
    ...
    cal_dist = {
        "FN": get_prob_distribution(fn_df["cal_prob"]),
    ...
    logger.info(f"Saved False Negative analysis results → {OUTPUT_PATH}")
