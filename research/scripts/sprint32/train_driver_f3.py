"""
scripts/sprint32/train_driver_f3.py

Sprint 32 Phase 3 — F3 trainer. Byte-for-byte the Sprint 31 training loop and
protocol (AdamW 1e-4, wd 1e-4, FocalLoss gamma 2 alpha 0.25, WeightedRandomSampler,
SHUFFLED val loader, clip 1.0, batch 64, 5000 steps/epoch, patience 3 on val TSS,
CosineAnnealingLR t_max=max_epochs, isotonic calibration deferred to eval) — the
ONLY change is the model class: LateFusionPatchTST (two-stream late fusion) in
place of the single-encoder PatchTST. Feature file must be the 36-channel F2
layout so the model's internal GOES(17)/Aditya(19) split is correct.
"""
...
from app.services.ml.model_f3 import LateFusionPatchTST, TOTAL_DIM
...
def main():
    ...
    ap.add_argument("--t-max", type=int, default=-1)
    ...
    assert len(feature_cols) == TOTAL_DIM, f"F3 expects {TOTAL_DIM} channels, got {len(feature_cols)}"
    t0 = time.time()
    set_seed(args.seed)
    out = os.path.join("artifacts", "sprint32", "runs", args.run_id)
    ...
    model = LateFusionPatchTST().to(device)
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        if m["tss"] > best_tss:
            ...
            torch.save({"epoch": epoch, "val_tss": best_tss, "best_threshold": float(thr),
                        ...
                        "n_features": TOTAL_DIM, "arch": "LateFusionPatchTST"}, os.path.join(out, "best.pt"))
        else:
            patience_ctr += 1
    ...
    meta = {"run_id": args.run_id, "seed": args.seed, "device": str(device),
            "arch": "LateFusionPatchTST", "n_features": TOTAL_DIM, "n_params": n_params,
    ...
    print(f"[{args.run_id}] DONE best_val_tss={best_tss:.4f}@ep{best_epoch} {meta['wall_seconds']}s", flush=True)
